...
export function EnhancedQuestionForm({
  ...
  const isMultipleChoice =
    selectedQuestionType?.input_type === 'multiple_choice' ||
    selectedQuestionType?.input_type === 'single_choice';
  const isReorderParagraphs =
    selectedQuestionType?.name === 'reorder_paragraphs' ||
    selectedQuestionType?.response_type === 'sequence';
  const isMultipleSelection =
    selectedQuestionType?.input_type === 'multiple_choice';
  const isPTEReading = selectedQuestionType?.section_id === 'pte-reading';
  ...
  const loadInitialData = async () => {
    try {
      await Promise.all([fetchMedia(), fetchQuestionTypes()]);
    } catch (error) {
  ...
  const handleQuestionTypeChange = (questionTypeId: string) => {
    ...
    setFormData((prev) => ({
      ...
      instructions: getDefaultInstructions(questionType),
      time_limit_seconds: getDefaultTimeLimit(questionType),
    }));
  ...
  const getDefaultInstructions = (questionType: any) => {
    if (!questionType) return '';

    if (questionType.name === 'reorder_paragraphs') {
    ...
    if (questionType.input_type === 'multiple_choice') {
    ...
    if (questionType.input_type === 'single_choice') {
  ...
  const getDefaultTimeLimit = (questionType: any) => {
    if (!questionType) return '';

    if (questionType.time_limit_seconds) {
      return questionType.time_limit_seconds.toString();
    }
    if (questionType.name === 'reorder_paragraphs') return '150';
    if (questionType.input_type === 'multiple_choice') return '150';
    if (questionType.input_type === 'single_choice') return '120';
    return '';
  };

  const getDefaultOptions = (questionType: any) => {
    if (!questionType) return [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.content || !formData.question_type_id) {
      toast.error('Title, content, and question type are required');
      return;
    }
  ...
  // Group question types by exam and section
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const groupedQuestionTypes = questionTypes.reduce((acc, qt) => {
    const examName = qt.sections?.exams?.display_name || 'Unknown Exam';
    const sectionName = qt.sections?.display_name || 'Unknown Section';
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        disabled={loading || questionTypesLoading}
                        required
                      >
                        <SelectTrigger>
                                ...
                                {(types as any[]).map((questionType: any) => (
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      <span>{questionType.display_name}</span>
                                      <Badge
                                        variant="outline"
                                        className="text-xs"
                                      >
                                        {questionType.input_type}
                                      </Badge>
                                    </div>
                                  </SelectItem>
                                ))}
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <h4 className="font-medium text-blue-900">
                                {selectedQuestionType.display_name}
                              </h4>
                              ...
                              <div className="flex gap-2 mt-2">
                                <Badge variant="outline" className="text-xs">
                                  Input: {selectedQuestionType.input_type}
                                </Badge>
                                <Badge variant="outline" className="text-xs">
                                  Response: {selectedQuestionType.response_type}
                                </Badge>
                                {selectedQuestionType.time_limit_seconds && (
                                  <Badge variant="outline" className="text-xs">
                                    Time:{' '}
                                    {selectedQuestionType.time_limit_seconds}s
                                  </Badge>
                                )}
                              </div>
                            </div>
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-blue-700 mt-1">
                            This question type (
                            {selectedQuestionType.display_name}) doesn't require
                            predefined answer options. Students will provide
          ...
          <div className="flex justify-end space-x-2 pt-4 border-t">
            ...
            <Button
              type="submit"
              disabled={loading || !formData.question_type_id}
            >
              {loading ? (
                <>
