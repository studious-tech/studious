...
export default function PTESpeakingRepeatSentence({
  ...
  useEffect(() => {
    return () => {
      ...
      if (sentenceAudioRef.current) {
        ...
        sentenceAudioRef.current.remove();
  ...
  const playSentenceAudio = useCallback(async () => {
    if (!sentenceAudioUrl) {
      console.warn(
        'No sentence audio URL available, skipping to preparation phase'
      );
      setPhase('preparing');
    ...
    try {
      ...
      sentenceAudioRef.current = new Audio(sentenceAudioUrl);

      sentenceAudioRef.current.onerror = (e) => {
        console.error('Error loading sentence audio:', e);
        toast.error('Failed to load sentence audio');
        setPhase('preparing');
        setPreparationTime(preparationSeconds);
      };
      ...
      if (error instanceof Error && error.name === 'NotAllowedError') {
        ...
        toast.error('Failed to play sentence audio');
      }
    }
  }, [sentenceAudioUrl, preparationSeconds]);
  ...
  const stopRecording = useCallback(() => {
    if (mediaRecorderRef.current && isRecording) {
      ...
      // Clear the recording countdown timer immediately
      if (recordingTimerRef.current) {
  ...
  // User must click play button to start sentence audio
  ...
  const progressPercent = () => {
    // Don't show progress if completed and not playing
            ...
            >
              {phase === 'playing' ? (
                <>
                  ...
                  >
                    Click the button below to play the sentence
                  </div>
                  <button
